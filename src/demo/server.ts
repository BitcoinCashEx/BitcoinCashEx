    ...
    <script>
      ...
      const refresh = async () => {
        ...
        launchProofPackView.innerHTML = launchPack.status === 'missing'
          ...
          : '<table><thead><tr><th>Status</th><th>Token Category</th><th>Graduation BCH</th><th>Pool BCH</th><th>Graduation Tokens</th><th>Pool Tokens</th><th>Token Binding</th><th>Token Source</th><th>Graduation</th><th>Pool</th><th>Pool Funding</th><th>Funding Outpoint</th><th>AMM Proof</th></tr></thead><tbody><tr><td><span class="' + (launchPack.status === 'verified' ? 'ok' : 'bad') + '">' + launchPack.status + '</span></td><td><code>' + (launchPack.tokenCategory || '') + '</code></td><td>' + amount(launchPack.graduationBchAmountSats || '') + '</td><td>' + amount(launchPack.migratedPoolBchSats || '') + '</td><td>' + amount(launchPack.graduationTokenAmount || '') + '</td><td>' + amount(launchPack.migratedPoolTokenAmount || '') + '</td><td>' + maybeTxLink(launchPack.tokenBindingTxid) + '</td><td><span class="' + (launchPack.tokenGenesisSourceConfirmed ? 'ok' : 'bad') + '">' + (launchPack.tokenGenesisSourceConfirmed ? 'verified' : 'missing') + '</span></td><td>' + maybeTxLink(launchPack.graduationTxid) + '</td><td>' + maybeTxLink(launchPack.poolTxid) + '</td><td><span class="' + (launchPack.poolFundingConfirmed ? 'ok' : 'bad') + '">' + (launchPack.poolFundingConfirmed ? 'verified' : 'missing') + '</span></td><td><code>' + poolFundingOutpoint + '</code></td><td>' + launchPack.ammProofPack.auditTxids.map(txLink).join(' ') + '</td></tr></tbody></table>' +
