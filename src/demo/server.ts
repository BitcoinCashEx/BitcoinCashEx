    ...
    <script>
      ...
      const refresh = async () => {
        ...
        metrics.innerHTML = [
          ...
          metric('Token output', tokenOutputStatus),
          metric('Token genesis source', launchPack.tokenGenesisSourceConfirmed ? 'verified' : launchPack.status === 'missing' ? 'missing' : 'failed'),
        ...
        launchProofPackView.innerHTML = launchPack.status === 'missing'
          ...
          : '<table><thead><tr><th>Status</th><th>Token Category</th><th>Graduation BCH</th><th>Pool BCH</th><th>Graduation Tokens</th><th>Pool Tokens</th><th>Token Conservation</th><th>Token Change</th><th>Token Binding</th><th>Token Source</th><th>Token Order</th><th>Graduation</th><th>Pool</th><th>Pool Funding</th><th>Funding Outpoint</th><th>AMM Proof</th></tr></thead><tbody><tr><td><span class="' + (launchPack.status === 'verified' ? 'ok' : 'bad') + '">' + launchPack.status + '</span></td><td><code>' + (launchPack.tokenCategory || '') + '</code></td><td>' + amount(launchPack.graduationBchAmountSats || '') + '</td><td>' + amount(launchPack.migratedPoolBchSats || '') + '</td><td>' + amount(launchPack.graduationTokenAmount || '') + '</td><td>' + amount(launchPack.migratedPoolTokenAmount || '') + '</td><td><span class="' + (launchPack.migrationTokenSupplyConserved ? 'ok' : 'bad') + '">' + migrationTokenStatus + '</span></td><td>' + amount(launchPack.migrationTokenChangeAmount || '') + ' / ' + amount(launchPack.expectedMigrationTokenChangeAmount || '') + '</td><td>' + maybeTxLink(launchPack.tokenBindingTxid) + '</td><td><span class="' + (launchPack.tokenGenesisSourceConfirmed ? 'ok' : 'bad') + '">' + (launchPack.tokenGenesisSourceConfirmed ? 'verified' : 'missing') + '</span></td><td><span class="' + (launchPack.tokenGenesisMinedBeforeBinding ? 'ok' : 'bad') + '">' + (launchPack.tokenGenesisMinedBeforeBinding ? 'verified' : 'failed') + '</span></td><td>' + maybeTxLink(launchPack.graduationTxid) + '</td><td>' + maybeTxLink(launchPack.poolTxid) + '</td><td><span class="' + (launchPack.poolFundingConfirmed ? 'ok' : 'bad') + '">' + (launchPack.poolFundingConfirmed ? 'verified' : 'missing') + '</span></td><td><code>' + poolFundingOutpoint + '</code></td><td>' + launchPack.ammProofPack.auditTxids.map(txLink).join(' ') + '</td></tr></tbody></table>' +
